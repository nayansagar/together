...
public class HttpUtils {
    ...
    public List<Map<String, String>> getActivities(String groupId) throws IOException {
        HttpGet httpGet = new HttpGet(hostName+"groups/"+groupId+"/activities/");
        HttpResponse httpResponse = httpClient.execute(httpGet);
        if(httpResponse.getStatusLine().getStatusCode() != 200){
            throw new RuntimeException("Failed to get activities for group "+groupId+", status : "+httpResponse.getStatusLine().getStatusCode());
        }
        String responseBody = convertStreamToString(httpResponse.getEntity().getContent());
